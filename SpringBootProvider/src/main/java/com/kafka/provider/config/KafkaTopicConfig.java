package com.kafka.provider.config;

import org.apache.kafka.clients.admin.NewTopic;
import org.apache.kafka.common.config.TopicConfig;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.kafka.config.TopicBuilder;

import java.util.HashMap;
import java.util.Map;

@Configuration
public class KafkaTopicConfig {

    @Bean
    public NewTopic generateTopic() {

        Map<String, String> configurations = new HashMap<>();
        // DELETE erase the message and COMPACT conserve the last one
        configurations.put(TopicConfig.CLEANUP_POLICY_CONFIG, TopicConfig.CLEANUP_POLICY_DELETE);

        // Retention Time in milliseconds, by default is -1 (NEVER erase the message)
        configurations.put(TopicConfig.RETENTION_MS_CONFIG, "86400000");

        // Max Sizes by segment, by default 1 GB
        configurations.put(TopicConfig.SEGMENT_BYTES_CONFIG, "1073741824");

        // Message size, by default 1 MB
        configurations.put(TopicConfig.MAX_MESSAGE_BYTES_CONFIG, "100012");

        return TopicBuilder.name("notification-topic")
                .partitions(2)
                .replicas(2)
                .configs(configurations)
                .build();
    }
}
